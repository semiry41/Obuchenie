...
def josephus_task(num_people, kill_num):
    data_list = [i for i in range(1, num_people + 1)]
    last_kill_index = 0
    while len(data_list) > 1:
        last_kill_index = (last_kill_index + kill_num - 1) % len(data_list) #  Нормализуем индекс в пределах длины списка
        data_list.pop(last_kill_index)
        print(last_kill_index)
    survivor = data_list[0]
